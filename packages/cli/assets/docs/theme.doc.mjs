  ...
  return (
    ...
    },
    {
      ...
      content: [
        {
          type: 'prose',
          text: 'defineTheme creates a theme from token overrides and optional scale configs. Scale configs generate tokens from parameters. Explicit token overrides always take precedence over scale-generated values, token by token. localTokens accepts any valid CSS custom-property name; prefixes do not establish ownership. iconDefaultSize may select xsm, sm, md, or lg as the theme default for Icons without an explicit or nearer component-slot size; an omitted value retains Core md. The Material 3 theme chooses lg (24px at a 16px root), while an explicit Icon size="md" stays 20px. One caveat for the accent: overriding --color-accent in tokens re-points the reference tokens (--color-accent-muted, --color-text-accent, --color-icon-accent) but NOT --color-on-accent, which stays baked from the color.accent seed. To give each scheme its own accent with a consistent derived palette, pass a [light, dark] tuple to color.accent instead of overriding the token.',
        },
        {
          type: 'code',
          lang: 'tsx',
